...
uses Windows, Messages, Types, Classes, Graphics, Forms, Controls, StdCtrls,
  SysUtils,
...
type
  ...
  TChangeAction = (CALoadPattern, CALoadSample, CALoadOrnament, CAOrGen,
    ...
    CAInsertPosition, CADeletePosition, CAChangePositionListLoop,
    CAChangePositionValue,
    CAChangeToneTable, CAChangeFeatures, CAChangeHeader, CAChangeAuthor,
    CAChangeTitle,
    CAPatternInsertLine, CAPatternDeleteLine, CAPatternClearLine,
    CAPatternClearSelection,
  ...
  TChangeParams = record
    ...
    case TChangeAction of
      ...
      CAChangePatternSize, CAChangeSampleSize, CAChangeOrnamentSize: (Size:
        integer);
      CAChangeSampleLoop, CAChangeOrnamentLoop, CAChangePositionListLoop: (Loop:
        integer);
  ...
  TMDIChild = class(TForm)
    ...
    procedure SamplesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure SamplesKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure OrnamentsKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure OrnamentsKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure ChangeTracks(Pat, Line, Chan, CursorX, n: integer; Keyboard:
      boolean);
...
begin
  ...
  for i := 0 to Length(MainForm.SampleLineTemplates) - 1 do
    ListBox1.Items.Add(GetSampleString(MainForm.SampleLineTemplates[i], False,
      True));
  ...
  if i < j then
    i := j;
  PageControl1.ClientHeight := i + Tracks.Left + PatternsSheet.Top +
    PatternsSheet.Left;
...
begin
  ...
  if (SelX = CursorX) and (CursorY = Y1) then
    exit;
  Y2 := CursorY;
  if Y1 > Y2 then
  begin
  ...
  if X2 in [8, 22, 36] then
    W := 3;
  InvertRect(DC1, Rect((X1 + 3) * CelW, Y1 * CelH, (X2 + 3 + W) * CelW, (Y2 + 1)
    * CelH));
  if DC = 0 then
...
begin
  ...
  if (SelX = CursorX) and (SelY = Y2) then
    exit;
...
begin
  ...
  begin
    ...
    if MainForm.MDIChildCount <> 0 then
      if ((i mod TMDIChild(MainForm.ActiveMDIChild).VTMP.Initial_Delay) = 0) and (i < len) and MainForm.Saved_HighlightSpeed then SetBkColor(DC1, GetSysColor(COLOR_ACTIVECAPTION));

    if MainForm.Saved_DecBase then
    begin
      s := Format('%.2d', [i mod 100]) + loopstring;
      if i = 100 then s := '@1' + loopstring;
      if i = 200 then s := '@2' + loopstring;
    end
    else
    begin
      s := IntToHex(i, 2) + loopstring;
    end;
    ...
    begin
      if i = 0 then
        ...
        s := s +
          GetSampleString(MainForm.SampleLineTemplates[MainForm.CurrentSampleLineTemplate], True, True)
    end
    else
...
begin
  case CursorX of
    0..3: s :=
      'Envelope generator period (hexadecimal, range 0-FFFF). Set envelope type to 1-E.';
    5..6: s := 'Noise generator base period (hexadecimal, range 0-1F).';
    8, 22, 36: s :=
      'Note (from C-1 to B-8). Numpad 1-8 to octave. A to R-- (release).';
    12, 26, 40: s := 'Sample (1-9,A-V). Used with note or R--.';
    13, 27, 41: s :=
      'Envelope type (hex 1-E) or envelope off (F). 0th ornament can be set only with 1-F.';
    14, 28, 42: s :=
      'Ornament (hex 0-F). 0th ornament can be set only with envelope type or off (1-F).';
    15, 29, 43: s := 'Volume (hexadecimal 1-F). Use R-- instead of volume 0.';
    17, 31, 45: s :=
      'Spec.command(1-tone down,2-tone up,3-port,4-sam offset,5-orn offset,6-vibrato,9-env down,A-env up,B-speed).';
    18, 32, 46: s :=
      'Spec. commands 1-3,9-A delay (hexadecimal 1-F to period of changes, 0 to stop).';
    19, 33, 47: s :=
      'Hi digit of speccom 1-5,9-B parameter (hex 0-F) or 1st param of speccom 6 (1-F to sound on period, 0 to stop).';
    20, 34, 48: s :=
      'Lo digit of speccom 1-5,9-B param (0-F) or 2nd param of speccom 6 (1-F to sound off period, 0 to stop after sound on period).';
  end;
...
begin
  ...
  if VTMP.Patterns[Pat].Items[Line].Channel[Chan].Note >= 0 then
    PlVars[1].ParamsOfChan[Chan].Note :=
      VTMP.Patterns[Pat].Items[Line].Channel[Chan].Note;
...
procedure TMDIChild.TracksKeyDown(Sender: TObject; var Key: Word;
  ...
  begin
    if (TSWindow <> nil) and (TSWindow <> Self) then
      ...
      begin
        ...
        TSWindow.Show;
        TSWindow.SetFocus;
        if TSWindow.Tracks.CanFocus then
          TSWindow.Tracks.SetFocus;
      end;
  end;
  ...
  var
   old, y,n,sfreq, efreq,snote, enote, spos, epos : Integer;
   len, diff :Integer;
   chan:Integer;
  begin
    chan:= MainForm.ChanAlloc[(Tracks.CursorX - 8) div 14];
    y := Tracks.ShownFrom - Tracks.N1OfLines + Tracks.CursorY;
    spos := y;
    snote :=  VTMP.Patterns[PatNum].Items[spos].Channel[Chan].Note;
    if snote = 0 then Exit;
    epos:=y;
    enote:= snote;
    ...
    begin
      ...
      if enote >= 0  then Break;
    end;
    if (enote < 0)or (enote = snote) then Exit;
    ...
    begin
      ...
      begin
        ...
        if DoStep(i, True) then
          ShowStat;
        RedrawTrs;
        ChangeList[ChangeCount - 1].NewParams.prm.PatternCursorY :=
          Tracks.CursorY;
        ChangeList[ChangeCount - 1].NewParams.prm.PatternShownFrom :=
          Tracks.ShownFrom;
      end;
    end;
  end;
        ...
        for c := 0 to 2 do
          ...
          begin
            for j := i + 1 to MaxPatLen - 1 do
              Tracks.ShownPattern.Items[j - 1].Channel[c] :=
                Tracks.ShownPattern.Items[j].Channel[c];
            Tracks.ShownPattern.Items[MaxPatLen - 1].Channel[c] :=
              EmptyChannelLine
          end;
        CalcTotLen;
        if DoStep(i, True) then
...
//      SpeedButton9Click(Sender);
        ...
        VTMP.IsChans[1].Global_Ton and
          VTMP.IsChans[1].Global_Noise and
          VTMP.IsChans[1].Global_Envelope
          then
        begin
...
//        SpeedButton1Click(Sender);
      ...
      begin
        //noise
        if VTMP.IsChans[0].Global_Noise and
          VTMP.IsChans[1].Global_Noise and
          VTMP.IsChans[2].Global_Noise then
        begin
          VTMP.IsChans[0].Global_Noise := False;
          VTMP.IsChans[1].Global_Noise := False;
          VTMP.IsChans[2].Global_Noise := False;
      ...
      begin
        //envelope
        if VTMP.IsChans[0].Global_Envelope and
          VTMP.IsChans[1].Global_Envelope and
          VTMP.IsChans[2].Global_Envelope then
        begin
          VTMP.IsChans[0].Global_Envelope := False;
...
//    TransposeSelection(12);
      ...
      //SelectPosition(StringGrid1.Col+1)
        if StringGrid1.Col < StringGrid1.ColCount then StringGrid1.Col := StringGrid1.Col + 1
      end
      else
      begin
      //next pattern
//      ChangePattern(PatNum+1);
        if PatNum <= 83 then Edit2.Text := IntToStr(PatNum + 1);
      end;
    VK_SUBTRACT:
      if (ssCtrl in Shift) and (ssShift in Shift) then
      begin
...
//    TransposeSelection(12);
      ...
      //prev pattern
      //ChangePattern(PatNum-1);
        if PatNum >= 1 then Edit2.Text := IntToStr(PatNum - 1);
      end;
    VK_HOME:
      begin
        RemSel;
    ...
    VK_PRIOR:
      begin
        ...
        if ssCtrl in Shift then
        begin
        // somewhere into middle
          if Tracks.ShownPattern = nil then
            PLen := DefPatLen
          else
            PLen := Tracks.ShownPattern.Length;

          Tracks.ShownFrom := PLen div 2;
          Tracks.CursorY := Tracks.N1OfLines;
          if not (ssShift in Shift) then
            Tracks.RemoveSelection(0, True);
          HideCaret(Tracks.Handle);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              HideCaret(Tracks.Handle);
              Tracks.RedrawTracks(0);
              SetCaretPos(Tracks.CelW * (3 + Tracks.CursorX), Tracks.CelH *
                Tracks.CursorY);
              ShowCaret(Tracks.Handle)
            end
          end
            //cursor in the middle or in the first line?
          else if (Tracks.CursorY = Tracks.N1OfLines) or (Tracks.CursorY = 0)
            then
          begin
    ...
    VK_NEXT:
      begin
        ...
        begin
          ...
          if Tracks.ShownFrom - Tracks.N1OfLines + Tracks.CursorY = PLen - 1
            then
          begin
            if not (ssShift in Shift) then
            begin
          ...
          else if (Tracks.CursorY = Tracks.N1OfLines) or (Tracks.CursorY =
            Tracks.NOfLines - 1) then
          begin
    ...
    VK_DELETE:
      ...
      begin
        ...
        {and ((Tracks.SelX <> Tracks.CursorX) or //commented especially for EA
      (Tracks.SelY <> Tracks.ShownFrom - Tracks.N1OfLines + Tracks.CursorY))}then
      begin
    ...
    begin
      if (Shift = [ssCtrl]) and (Key = Ord('Y')) then
        DoRemoveLine
      else if (Shift = [ssCtrl]) and ((Key = Ord('A')) or (Key = VK_NUMPAD5))
        then
        Tracks.SelectAll
...
begin
  ...
  else if (Shift = [ssCtrl, ssShift]) or
    (Shift = [ssShift]) then
    if KeyPressed <> Key then
      if CursorX in NotePoses then
        DoNoteKey
end;
procedure TMDIChild.SamplesKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
 begin
   if (Shift <> [ssShift]) and (Samples.isLineTesting) and (not MainForm.Saved_TestForever) then
   begin
...
var
  ST: PSampleTick;
  ix, SLen, SPos: Integer;
  ff, ll, ii: Integer; //for, len , position
    ...
    begin
      ...
      with ShownSample.Items[i] do
        case n of
          ...
          TgAccNoise:
            Envelope_or_Noise_Accumulation := not
              Envelope_or_Noise_Accumulation;
          TgAccVol:
  ...
  procedure copySampleToBuffer();
  var
    ff:Integer;
  begin
     if not Samples.isSelecting then Exit;
     ValidateSample2(SamNum);
     GetSamParams(ll, ii);

//   ll:= Samples.selStart;
//   ii:= Samples.selEnd;
     for ff := 0 to (Samples.selEnd - Samples.selStart) do
     begin
       MainForm.BuffSample.Items[ff] :=
       Samples.ShownSample.items[ff + Samples.selStart];
     end;

     MainForm.BuffSample.Loop := 0;
     MainForm.BuffSample.Length := Samples.selEnd - Samples.selStart;
  end;

  procedure selectionOff();
  begin
//   copySampleToBuffer();
...
begin
//  if Shift <> [ssShift] then Samples.isSelecting := False;
//  if (Shift = [ssShift]) and
//     (isSelecting = False) and then

  if ((Shift <> [ssShift])) and (not MainForm.Saved_TestForever ) then
    begin
  ...
  if Shift = [] then
    case Key of
      ...
      VK_END:
        ...
        begin
          Samples.CursorX := 20;
          Samples.RecreateCaret;
          SetCaretPos(Samples.CelW * (3 + Samples.CursorX), Samples.CelH *
            Samples.CursorY);
        end;
      ...
      VK_DELETE:
        ...
        //Samples.CursorY;
          ...
          begin
            ...
            if Samples.ShownSample.Loop > ii then Samples.ShownSample.Loop := Samples.ShownSample.Loop - 1;
          end;
    ...
    case Key of
      VK_NEXT, VK_END, VK_DOWN:
        begin
          selectionOff;
          if (((Key = VK_END) or (Key = VK_DOWN)) and (Samples.CursorX <> 20)) or
      ...
      VK_PRIOR, VK_HOME, VK_UP:
        begin
          selectionOff;        
          if (((Key = VK_HOME) or (Key = VK_UP)) and (Samples.CursorX <> 0)) or
      ...
      VK_ADD:
        begin
        //next sample
          if StrToInt(Edit5.Text) in [1..30] then
          begin
      ...
      VK_SUBTRACT:
        begin
        //previous sample
          if StrToInt(Edit5.Text) in [2..31] then
          begin
            ChangeSample(StrToInt(Edit5.Text) - 1);
            Edit5.Text := IntToStr((StrToInt(Edit5.Text) - 1));
          end;
    ...
    case Key of
      ...
      VK_END:
        begin
          ...
          if Samples.ShownSample.Loop > Samples.ShownSample.Length then Samples.ShownSample.Loop := ii;
...
//          ii:= Samples.selEnd;
          ...
          for ff := MainForm.BuffSample.Loop to MainForm.BuffSample.Length-1 do
          begin
            if ff + ii <= 63 then
            begin
      ...
      VK_DOWN:
        begin
          if samples.isSelecting = False then
          begin
          GetSamParams(ll, ii);
          Samples.selStart:=ii;
          Samples.selEnd:=ii+1;
          ...
          if Samples.isSelecting = True then
          begin
          GetSamParams(ll, ii);
          Samples.selEnd:=ii+1;
          end;
      ...
      VK_UP:
        begin
          ...
          if Samples.selEnd = Samples.selStart then Inc(Samples.selEnd);
...
//        curToneTab:= TMDIChild(ActiveMDIChild).VTMP.Ton_Table;
          ...
          if Key >= 256 then
          exit;
          if MainForm.NoteKeys[Key] >= 0 then
          begin
        //noteFreq1:= GetNoteFreq(curToneTab,VTMP.Patterns[-1].Items[SamNum].Channel[0].Note);
          ...
          noteFreq2:= GetNoteFreq(curToneTab, MainForm.NoteKeys[Key]+ (12 *( SampleTestLine.TestOct - 1)));
...
procedure TMDIChild.OrnamentsKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
 begin
   if (Shift <> [ssShift]) and (Ornaments.isLineTesting) and (not MainForm.Saved_TestForever) then
   begin
...
var
    ...
    // you can edit everywhere
     { if i >= l then
      ...
      begin
        ...
        ChangeList[ChangeCount - 1].OldParams.prm.OrnamentCursor := c;
        ChangeList[ChangeCount - 1].OldParams.prm.OrnamentShownFrom :=
          ShownFrom;
      end;
      HideCaret(Handle);
      RedrawOrnaments(0);
  ...
  procedure copyOrnamentToBuffer();
  var
    ff:Integer;
  begin
     if not Ornaments.isSelecting then Exit;
     ValidateOrnament(OrnNum);
     GetOrnParams(ll, ii, cc);

     for ff := 0 to (Ornaments.selEnd - Ornaments.selStart) do
     begin
       MainForm.BuffOrnament.Items[ff] :=
       Ornaments.ShownOrnament.items[ff + Ornaments.selStart];
     end;

     MainForm.BuffOrnament.Loop := 0;
     MainForm.BuffOrnament.Length := Ornaments.selEnd - Ornaments.selStart;
  end;

  procedure selectionOff();
  begin
//   copyOrnamentToBuffer();
  ...
  if Shift = [] then
    case Key of
      VK_NEXT:
        begin
          ...
          SetCaretPos(Ornaments.CelW * (3 + Ornaments.CursorX), Ornaments.CelH *
            Ornaments.CursorY);
        end;
      VK_PRIOR:
        begin
      ...
      VK_DELETE:
        ...
        //Samples.CursorY;
          ...
          begin
            ...
            if Ornaments.ShownOrnament.Loop > ii then Ornaments.ShownOrnament.Loop := Ornaments.ShownOrnament.Loop - 1;
          end;
      ...
      begin
        ...
        if MainForm.NoteKeys[Key] >= 0 then
        begin
          Ornaments.ShownOrnament.Items[ii] := MainForm.NoteKeys[Key];
          HideCaret(Ornaments.Handle);
          Ornaments.RedrawOrnaments(0);
          ShowCaret(Ornaments.Handle);
    ...
    case Key of
      ...
      VK_ADD:
        begin
        //next ornament
          if StrToInt(Edit11.Text) in [1..14] then
          begin
            ChangeOrnament(StrToInt(Edit11.Text) + 1);
      ...
      VK_SUBTRACT:
        begin
        //previous sample
          if StrToInt(Edit11.Text) in [2..15] then
          begin
            ChangeSample(StrToInt(Edit11.Text) - 1);
            Edit11.Text := IntToStr((StrToInt(Edit11.Text) - 1));
          end;
        end;
{      VK_INSERT:
    ...
    case Key of
      ...
      VK_END:
        begin
          ...
          if Ornaments.ShownOrnament.Loop > Ornaments.ShownOrnament.Length then Ornaments.ShownOrnament.Loop := ii;
      ...
      VK_INSERT:
        begin
          ...
          for ff := MainForm.BuffOrnament.Loop to MainForm.BuffOrnament.Length do
          begin
            if ff + ii <= 254 then
            begin
              Ornaments.ShownOrnament.Items[ff + ii] := MainForm.BuffOrnament.Items[ff];
            //Ornaments.ShownOrnament.Length:= ff;
              if ff+ii >= Ornaments.ShownOrnament.Length then
                 Ornaments.ShownOrnament.Length:= ff+ii+1;
            end;
          end;
      ...
      VK_DOWN:
        begin
          ...
          Ornaments.selEnd:=ii+1;
          Ornaments.isSelecting:= True
          end;

          if (Ornaments.CursorY < Ornaments.NOfLines - 1) then
          begin
            Inc(Ornaments.CursorY);
            SetCaretPos(Ornaments.CelW * (3 + Ornaments.CursorX), Ornaments.CelH *
              Ornaments.CursorY);
          end
          else if Ornaments.ShownFrom < MaxSamLen - Ornaments.NOfLines then
          begin
            Inc(Ornaments.ShownFrom);
      ...
      VK_UP:
        begin
          ...
          if Ornaments.selEnd = Ornaments.selStart then Inc(Ornaments.selEnd);
        ...
        begin
         ...
         begin
           OrnamentTestLine.TestLineKeyDown(Sender, Key, Shift);
         end;
        end;
    end
  else if Shift = [ssShift, ssCtrl] then
    begin
        ...
        if (MainForm.NoteKeys[Key]+12 >= 0 )and
           (MainForm.NoteKeys[Key]+12 <= 96 )
            then
        begin
          Ornaments.ShownOrnament.Items[ii] := MainForm.NoteKeys[Key]+12;
          HideCaret(Ornaments.Handle);
          Ornaments.RedrawOrnaments(0);
          ShowCaret(Ornaments.Handle);
...
procedure TMDIChild.TracksMoveCursorMouse(X, Y: integer; Sel, Mv, ButRight:
  boolean);
var
...
begin
  ...
  begin
    ...
    Tracks.Clicked := (y1 >= i) and (y1 < i + PLen) and
...
begin
  ...
  TracksMoveCursorMouse(X, Y, GetKeyState(VK_SHIFT) and 128 <> 0, False, Shift =
    [ssRight]);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if i > l then
      begin
        ShownSample.Length:=i+1;
          Edit9.Text := IntToStr(ShownSample.Length);
          Edit10.Text := IntToStr(ShownSample.Loop);

          ShownSample.Items[i].Mixer_Ton:=True;
        ...
        with ShownSample.Items[i] do
          case x1 of
            ...
            17: Envelope_or_Noise_Accumulation := not
              Envelope_or_Noise_Accumulation;
...
begin
  ...
  begin
    ...
    x1 := X div Samples.CelW - 3;
    y1 := Y div Samples.CelH;
    i := Samples.ShownFrom + y1;
      if i >= Samples.ShownSample.Length then
      begin
        Samples.ShownSample.Length:=i+1;
          Edit9.Text := IntToStr(Samples.ShownSample.Length);
          Edit10.Text := IntToStr(Samples.ShownSample.Loop);

          Samples.ShownSample.Items[i].Mixer_Ton:=True;
...
begin
  WinFileName := Name;
  Caption := IntToStr(WinNumber) + ': ' + ExtractFileName(Name) + ' (' +
  ...
  for i := 0 to MainForm.MDIChildCount - 1 do
    if TMDIChild(MainForm.MDIChildren[i]).TSWindow = Self then
      TMDIChild(MainForm.MDIChildren[i]).TSBut.Caption :=
        PrepareTSString(TMDIChild(MainForm.MDIChildren[i]).TSBut, Caption);
end;
...
var
  ...
  function Convert(FType: Available_Types; VTMP: PModule; var VTMP2: PModule):
      boolean;
  begin
    Result := True;
    case FType of
      Unknown:
        begin
          ...
          begin
            Result := False;
            Close;
            MessageBox(MainForm.Handle, PChar(ers[i] + ' (line: ' +
              IntToStr(TxtLine) + ')'),
...
begin
  ...
  try
    if VTMP2 = nil then
    begin
      FileType := LoadAndDetect(@ZXP, Name, i, FType2, TSSize2, ZXAddr, Tm,
        Andsix, AuthN, SongN);
...
begin
  ...
  begin
    ...
    else if ps < StringGrid1.LeftCol then
      StringGrid1.LeftCol := ps;
    sel.Left := ps;
    sel.Right := ps;
    sel.Top := 0;
    sel.Bottom := 0;
...
begin
  ...
  begin
    ...
    ChangeList[ChangeCount - 1].NewParams.prm.PositionListLen :=
...
begin
  ...
  edtBPM.Text:= 'BPM: ' + FloatToStr((Interrupt_Freq*60 / (UpDown3.Position * 4))/1000);
end;
...
begin
  for ff := -1 to 84 do
  begin
    if VTMZ.Patterns[ff] = nil then Continue;
...
begin
  SongChanged := True;
  AddUndo(CAChangeHeader, integer(not VTMP.VortexModule_Header),
    SaveHead.ItemIndex);
...
begin
  ...
  begin
    ...
    if not UndoWorking then
    begin
      ChangeList[ChangeCount - 1].OldParams.prm.PrevLoop :=
      ...
      ChangeList[ChangeCount - 1].NewParams.prm.PrevLoop :=
        VTMP.Samples[SamNum].Loop;
...
begin
  if (VTMP.Samples[SamNum] = nil) then
    exit;
  if (NL <> VTMP.Samples[SamNum].Loop) and (NL < VTMP.Samples[SamNum].Length)
    then
  begin
    SongChanged := True;
...
begin
  if (VTMP.Ornaments[OrnNum] = nil) then
    exit;
  if (NL <> VTMP.Ornaments[OrnNum].Loop) and (NL < VTMP.Ornaments[OrnNum].Length)
    then
  begin
    SongChanged := True;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if s[7] <> #32 then
        begin
          if not SGetNumber(s[7], 15, i) then
            exit;
          nums[l, 5] := i
        end;
        for k := 0 to 2 do
        ...
        begin
          if nums[l, k] >= 256 - 2 then
            if not Merge or (nums[l, k] <> 255) then
              ShownPattern.Items[Y1 + l].Channel[MainForm.ChanAlloc[(m - 8) div
                14]].Note := nums[l, k] - 256
        end
        else
        begin
          if m = 5 then
          ...
          begin
            ...
            case m of
              ...
              12, 26, 40: if not Merge or (nums[l, k] <> 0) then
                  ShownPattern.Items[Y1 + l].Channel[sz].Sample := nums[l, k];
              13, 27, 41: if not Merge or (nums[l, k] <> 0) then
                  ShownPattern.Items[Y1 + l].Channel[sz].Envelope := nums[l, k];
              14, 28, 42: if not Merge or (nums[l, k] <> 0) then
                  ShownPattern.Items[Y1 + l].Channel[sz].Ornament := nums[l, k];
              15, 29, 43: if not Merge or (nums[l, k] <> 0) then
                  ShownPattern.Items[Y1 + l].Channel[sz].Volume := nums[l, k];
              17, 31, 45: newc[sz].Number := nums[l, k];
              18, 32, 46: newc[sz].Delay := nums[l, k];
              19, 33, 47: newc[sz].Parameter := newc[sz].Parameter and 15 or
                (nums[l, k] shl 4);
              20, 34, 48: newc[sz].Parameter := newc[sz].Parameter and $F0 or
                nums[l, k];
            end;
          end;
        end;
    ...
    begin
      ...
      repeat
        ...
        begin
          ...
          case m of
            0: ShownPattern.Items[Y1].Envelope := ShownPattern.Items[Y1].Envelope
              and $FFF;
            1: ShownPattern.Items[Y1].Envelope := ShownPattern.Items[Y1].Envelope
              and $F0FF;
            2: ShownPattern.Items[Y1].Envelope := ShownPattern.Items[Y1].Envelope
              and $FF0F;
            3: ShownPattern.Items[Y1].Envelope := ShownPattern.Items[Y1].Envelope
              and $FFF0;
            5: ShownPattern.Items[Y1].Noise := ShownPattern.Items[Y1].Noise and
              15;
            6: ShownPattern.Items[Y1].Noise := ShownPattern.Items[Y1].Noise and
              $F0;
            ...
            19, 33, 47:
              ShownPattern.Items[Y1].Channel[c].Additional_Command.Parameter :=
                ShownPattern.Items[Y1].Channel[c].Additional_Command.Parameter and
                15;
            20, 34, 48:
              ShownPattern.Items[Y1].Channel[c].Additional_Command.Parameter :=
                ShownPattern.Items[Y1].Channel[c].Additional_Command.Parameter and
...
begin
  ...
  begin
    Action := CA;
    case CA of
      CAChangeSpeed, CAChangeToneTable, CAChangePositionListLoop,
        CAChangeFeatures,
        CAChangeHeader:
        begin
      ...
      CAChangePatternSize, CAChangeNote, CAChangeEnvelopePeriod, CAChangeNoise,
        ...
        CAChangeSpecialCommandParameter, CALoadPattern,
        CAInsertPatternFromClipboard,
        CAPatternInsertLine, CAPatternDeleteLine, CAPatternClearLine,
        CAPatternClearSelection,
        CATransposePattern, CATracksManagerCopy, CAExpandCompressPattern:
        begin
      ...
      CALoadOrnament, CAOrGen:
        begin
          OldParams.prm.OrnamentCursor := Ornaments.CursorY + Ornaments.CursorX
            div 7 * OrnNRaw;
    ...
    begin
      ...
      with ChangeList[index] do
        case Action of
          ...
          CAChangeNote:
            begin
              ChangeNote(ComParams.CurrentPattern, Line, Channel,
                Pars.prm.Note);
          ...
          CAChangeEnvelopePeriod:
            begin
              ...
              if j > 3 then
                j := 0;
              ChangeTracks(ComParams.CurrentPattern, Line, Channel, j,
                Pars.prm.Value, False);
              RedrawTracks(index, @OldParams, False);
            end;
          CAChangeNoise, CAChangeSample, CAChangeEnvelopeType,
            CAChangeOrnament, CAChangeVolume, CAChangeSpecialCommandDelay:
            begin
              ChangeTracks(ComParams.CurrentPattern, Line, Channel,
                PatternCursorX, Pars.prm.Value, False);
              RedrawTracks(index, @OldParams, False);
            end;
          ...
          CAChangeSampleValue:
            begin
              ...
              ST := SampleLineValues^;
              SampleLineValues^ :=
                VTMP.Samples[ComParams.CurrentSample].Items[Line];
  ...
  begin
    if MainForm.SaveDialog1.FilterIndex = 1 then
      MainForm.SaveDialog1.FileName :=
        ChangeFileExt(MainForm.SaveDialog1.FileName, '.txt')
    else
      MainForm.SaveDialog1.FileName :=
        ChangeFileExt(MainForm.SaveDialog1.FileName, '.pt3');
    if MainForm.AllowSave(MainForm.SaveDialog1.FileName) then
    begin
      SetFileName(MainForm.SaveDialog1.FileName);
      MainForm.SavePT3(Self, MainForm.SaveDialog1.FileName,
...
begin
  Result := s;
  DC := GetDC(Handle);
  GetTextExtentExPoint(DC, PChar(s), Length(s), TSBut.ClientWidth, @nch, nil,
    Sz);
  ReleaseDC(Handle, DC);
...
begin
  ...
  if (TSSel.ShowModal = mrOk) and (TSSel.ListBox1.ItemIndex >= 0) and
    (TMDIChild(TSSel.ListBox1.Items.Objects[TSSel.ListBox1.ItemIndex]) <> Self)
    then
  begin
    TSBut.Caption := PrepareTSString(TSBut,
      TSSel.ListBox1.Items[TSSel.ListBox1.ItemIndex]);
    if (TSWindow <> nil) and (TSWindow <> Self) then
    begin
      TSWindow.TSBut.Caption := PrepareTSString(TSWindow.TSBut,
        TSSel.ListBox1.Items[0]);
      TSWindow.TSWindow := nil;
    end;
    TSWindow :=
      TMDIChild(TSSel.ListBox1.Items.Objects[TSSel.ListBox1.ItemIndex]);
    if (TSWindow <> nil) and (TSWindow <> Self) then
    begin
...
begin
  case Key of
    VK_PRIOR:
      begin
        i := UpDown5.Position + Tracks.HLStep;
        if i > MaxPatLen then
          i := MaxPatLen;
        UpDown5.Position := i;
      end;
...
procedure TMDIChild.chk1Click(Sender: TObject);
begin
  MainForm.Saved_EnvelopeAsNote := chk1.Checked;
  Tracks.RedrawTracks(0);
  if Tracks.Focused then
    ShowCaret(Tracks.Handle);
  if Active then
